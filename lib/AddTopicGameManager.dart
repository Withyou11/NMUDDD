...
class AddTopicGameManager extends StatefulWidget {
  const AddTopicGameManager({Key? key, required this.list}) : super(key: key);
  final List<GameTopicModel> list;
  @override
  State<AddTopicGameManager> createState() =>
      _AddTopicGameManagerState();
}

class _AddTopicGameManagerState extends State<AddTopicGameManager> {
  ...
  Future<void> addTopic(
      BuildContext context, String image,String nameId, List<String>? list) {
    CollectionReference addStorage =
        FirebaseFirestore.instance.collection('getTopicGame');
    return addStorage.doc(nameId).set({
      'image': image,
      'listUser': list,
    }).then((doc){
      Navigator.of(context).push(MaterialPageRoute(
          builder: (context) => AddDataTopicGame(nameDoc: nameId)));
    })
                                    ...
                                    GestureDetector(
                                        onTap: () {
                                          if (url != '' &&
                                            ...
                                            showNotifi(context,
                                                'Please enter enough information or NameTopic was existed');
                                          }
                                        },
                                        child: Container(
                                          width: 70,
  ...
  bool check(){
    for(int i=0;i< widget.list.length;i++){
      if(widget.list[i].nameId==_nameController.text) {
        return true;
      }
    }
    return false;
